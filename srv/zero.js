...
case 'asupan':
									...
									reply(mess.wait)
									data = fs.readFileSync('./mess/asupan.js');
									jsonData = JSON.parse(data);
									randIndex = Math.floor(Math.random() * jsonData.length);
									randKey = jsonData[randIndex];
									p = await getBuffer(randKey.result)
									zero.sendMessage(from, p, video, { quoted:  ftrol, mimetype: 'video/mp4', caption: 'Subscribe channel Leongaz'})
									break
case 'asupancecan':
...
case 'thailand':
reply(mess.wait)
ini_result = await fetchJson(`https://zeroyt7-api.xyz/cewek${command}?apikey=Il8lFnP6`)
